...
function list_unit() {
    ...
    var success = function(result) {
        ...
        if (json_data == "")
            div.append('<span class="title" style="color:red">Chưa có câu hỏi cho kì thi và nhóm câu hỏi đã chọn, vui lòng thêm câu hỏi trước khi tạo đề thi, thêm câu hỏi <a href="them-cau-hoi">tại đây</a>!</span>');
        else {
            for (var i = 0; i < json_data.length; i++) {
                var unit_div = $('<div class="input-level row col s12" id="unit_' + json_data[i].idnhomcauhoi + '"><span class="col s12"><b>Nhóm ' + json_data[i].vanbanbotro + '</b></span></div>');
