...
class Controller_Admin
{
    ...
    public function __construct()
    {
        ...
        $this->info['admin_id'] = $user_info->idqtv;
        $this->info['name'] = $user_info->hoten;
        $this->info['username'] = $user_info->tentaikhoan;
    }
    ...
    {
        $model = new Model_Admin();
        echo json_encode($model->get_list_nhomcauhoi());
    }
    ...
    private function add_admin($vanbanbotro, $diemtoidanhomcauhoi, $loai)
    {
        $model = new Model_Admin();
        return $model->add_admin($vanbanbotro, $diemtoidanhomcauhoi, $loai);
    }
    ...
    private function add_student($tentaikhoan, $matkhau, $hoten, $idkythi, $email, $ngaysinh, $sodienthoai)
    {
        $model = new Model_Admin();
        return $model->add_student($tentaikhoan, $matkhau, $hoten, $idkythi, $email, $ngaysinh, $sodienthoai);
    }
    ...
    {
        $model = new Model_Admin();
        return $model->add_class($grade_id, $class_name, $teacher_id, $this->info['admin_id'] );
    }
    ...
    {       
        $result = array();
        $vanbanbotro = isset($_POST['vanbanbotro']) ? Htmlspecialchars(addslashes($_POST['vanbanbotro'])) : '';
        $diemtoidanhomcauhoi = isset($_POST['diemtoidanhomcauhoi']) ? Htmlspecialchars(addslashes($_POST['diemtoidanhomcauhoi'])) : '';
        $loai = isset($_POST['loai']) ? Htmlspecialchars(addslashes($_POST['loai'])) : '';
        if (empty($vanbanbotro)||empty($diemtoidanhomcauhoi)||empty($loai)) {
            $result['status_value'] = "Không được bỏ trống các trường nhập!";
            $result['status'] = 0;
        } else {
            $add = $this->add_admin($vanbanbotro, $diemtoidanhomcauhoi, $loai);
            if ($add) {
    ...
    {
        $result = array();
        $tenkythi = isset($_POST['tenkythi']) ? Htmlspecialchars(addslashes($_POST['tenkythi'])) : '';
        $mota = isset($_POST['mota']) ? Htmlspecialchars(addslashes($_POST['mota'])) : '';
        $matkhau = isset($_POST['matkhau']) ? Htmlspecialchars(addslashes($_POST['matkhau'])) : '';
        if (empty($tenkythi)||empty($mota)||empty($matkhau)) {
            $result['status_value'] = "Không được bỏ trống các trường nhập!";
            $result['status'] = 0;
        } else {
            $add = $this->add_class($tenkythi, $mota, $matkhau, $this->info['idqtv']);
            if ($add) {
                $result['status_value'] = "Thêm thành công!";
                $result['status'] = 1;
            } else {
    ...
    {
        $result = array();
        $class_id = Htmlspecialchars($_POST['idkythi']);
    ...
    {
        $result = array();
        $hoten = isset($_POST['hoten']) ? Htmlspecialchars(addslashes($_POST['hoten'])) : '';
        $tentaikhoan = isset($_POST['tentaikhoan']) ? Htmlspecialchars(addslashes($_POST['tentaikhoan'])) : '';
        $idkythi = isset($_POST['idkythi']) ? Htmlspecialchars(addslashes($_POST['idkythi'])) : '';
        $email = isset($_POST['email']) ? Htmlspecialchars(addslashes($_POST['email'])) : '';
        $ngaysinh = isset($_POST['ngaysinh']) ? Htmlspecialchars(addslashes($_POST['ngaysinh'])) : '';
        $matkhau = isset($_POST['matkhau']) ? Htmlspecialchars(addslashes($_POST['matkhau'])) : '';
        $sodienthoai = isset($_POST['sodienthoai']) ? Htmlspecialchars(addslashes($_POST['sodienthoai'])) : '';
        if (empty($hoten)||empty($tentaikhoan)||empty($matkhau)) {
            $result['status_value'] = "Không được bỏ trống các trưòng nhập";
            $result['status'] = 0;
        } else {
            $add = $this->add_student($tentaikhoan, $matkhau, $hoten, $idkythi, $email, $ngaysinh, $sodienthoai);
            if ($add) {
                $result['status_value'] = "Thêm thành công!";
                $result['status'] = 1;
            } else {
    ...
    public function check_add_question()
    {
        ...
        $mark = isset($_POST['unit']) ? $_POST['unit'] : '';
        $subject_id = isset($_POST['subject_id']) ? $_POST['subject_id'] : '';
        ...
        $answer_f = isset($_POST['answer_f']) ? $_POST['answer_f'] : '';
        $correct_answer = isset($_POST['correct_answer']) ? $_POST['correct_answer'] : '';

        $true_correct_answer = "";
        if (empty($question_detail)||empty($subject_id)||empty($mark)||empty($answer_a)||empty($answer_b)||empty($answer_c)||empty($answer_d)||empty($correct_answer)) {
            $result['status_value'] = "Không được bỏ trống các trường nhập";
            $result['status'] = 0;
        } else {
    ...
    {
        $result = array();
        $tenbaithi = isset($_POST['tenbaithi']) ? Htmlspecialchars(addslashes($_POST['tenbaithi'])) : '';
        $motabaithi = isset($_POST['motabaithi']) ? Htmlspecialchars(addslashes($_POST['motabaithi'])) : '';
        $passwordbaithi = isset($_POST['passwordbaithi']) ? Htmlspecialchars(addslashes($_POST['passwordbaithi'])) : '';
        $idnhomcauhoi = isset($_POST['subject_id']) ? Htmlspecialchars(addslashes($_POST['subject_id'])) : '';
        $thoiluongbaithi = isset($_POST['thoiluongbaithi']) ? Htmlspecialchars(addslashes($_POST['thoiluongbaithi'])) : '';
        $loaibaithi = isset($_POST['loaibaithi']) ? Htmlspecialchars(addslashes($_POST['loaibaithi'])) : '';
        $thoigianchophepbatdau = isset($_POST['thoigianchophepbatdau']) ? Htmlspecialchars(addslashes($_POST['thoigianchophepbatdau'])) : '';
        $thoigiandongbaithi = isset($_POST['thoigiandongbaithi']) ? Htmlspecialchars(addslashes($_POST['thoigiandongbaithi'])) : '';
        $diemtoida = isset($_POST['diemtoida']) ? Htmlspecialchars(addslashes($_POST['diemtoida'])) : '';
        $idkythi = isset($_POST['grade_id']) ? Htmlspecialchars(addslashes($_POST['grade_id'])) : '';
        if (empty($tenbaithi)||empty($thoiluongbaithi)||empty($idkythi)) {
            $result['status_value'] = "Không được bỏ trống các trường nhập!";
            $result['status'] = 0;
        } else {
                $this->add_test(
